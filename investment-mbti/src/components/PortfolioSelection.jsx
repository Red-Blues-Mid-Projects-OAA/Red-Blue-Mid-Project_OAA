...
function PortfolioSelection({ recommendedStocks = [], finalLevel = 2, lambdaFinal = 7.7, savedSelectedTickers, onSelectedTickersChange, onBack, onRestart, onConfirm }) {
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetch(buildApiUrl('/api/all-stocks'), { headers: { 'Bypass-Tunnel-Reminder': 'true' } }).then(r => r.json()),
            fetch(buildApiUrl('/api/last-updated'), { headers: { 'Bypass-Tunnel-Reminder': 'true' } }).then(r => r.json()).catch(() => null),
