...
def _get_search_space(trial: optuna.Trial, profile: str) -> tuple[dict, str]:
    ...
    if profile == TSM_GATE_PROFILE:
        return {
            ...
            "l1_ratio": trial.suggest_float("l1_ratio", 0.0, 1.0),
            "tol": trial.suggest_float("tol", 1e-6, 5e-3, log=True),
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for offset in range(N_STRIDE_MODELS):
            ...
            params = {
                ...
                "penalty": "elasticnet",
                "C": float(search["C"]),
                "l1_ratio": float(search["l1_ratio"]),
                "tol": float(search["tol"]),
                "class_weight": search.get("class_weight"),
                "random_state": 42,
                "max_iter": 5000,
            }
        ...
        for direction_mode in direction_candidates:
            ...
            candidate = {
                ...
                "val_ic": float(val_ic),
                "gap": float(gap_abs),  # backward-compat alias
...
def optimize(
    ...
    params_path = get_model_params_path("logreg", ticker)
    ...
    study = optuna.create_study(
        direction="minimize",
        study_name=f"logreg_{profile}_{resolved_stage}_{CV_MODE}",
        sampler=sampler,
    )
    study.optimize(
    ...
    print(f"  Val Proba Std        : {best_val_proba_std:.6f}")
